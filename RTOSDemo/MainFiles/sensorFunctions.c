...
int frontWallStatus(RoverControlStruct *roverControlData, double thresholdAnglePollTotal, uint8_t thresholdAnglePollCount){
	...
	else if(roverControlData->sensorDistance[FRONT_LEFT_MEDIUM_SENSOR] < FRONT_AQUIRE_ANGLE_DISTANCE && roverControlData->sensorDistance[FRONT_LEFT_MEDIUM_SENSOR] > (FRONT_STOP_DISTANCE + thresholdAdjustment + 8)){ // between 10 inches and 15 inches
...
void distanceToEncoder(double distance, uint8_t* revolutions, uint16_t* ticksOffset){
	distance = distance/WHEEL_CIRCUMFERENCE;
	*revolutions = (uint8_t)(distance);
	*ticksOffset = (uint16_t)((distance - *revolutions)*TICKS_PER_REVOLUTION);
}