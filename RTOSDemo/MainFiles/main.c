...
static vtConductorStruct conductorData_uart;
#endif
...
int main( void )
{
	...
	if (initUART(&uart1, 1, mainUARTMONITOR_TASK_PRIORITY, 19200, UART_PARITY_NONE, UART_DATABIT_8, UART_STOPBIT_1) != UART_INIT_SUCCESS) {
		VT_HANDLE_FATAL_ERROR(0);
	}
	
	startTimerForUART(&uart1);
	
	vStartConductorTask_UART(&conductorData_uart,mainCONDUCTOR_TASK_PRIORITY,&uart1);
